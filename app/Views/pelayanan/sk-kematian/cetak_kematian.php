...
<body>
  ...
  <center>
    <p style="margin:0;"><b><u>SURAT KETERANGAN KEMATIAN</u></b></p>
  </center>
  <center>
  <?php foreach ($sk as $sk) : ?>
    <p style="margin:0;">Nomor : 02.0<?= $sk->kematian + 1 ?>/DM-SKM/<?=$bul?>/<?=$tahun?></p>
    <?php endforeach ?>
  </center>
  <div id=halaman>
    <p>Yang bertanda tangan dibawah ini Kepala Desa Daya Murni menerangkan bahwa :</p>
    <table style="margin-left:20;">
      <?php foreach ($kematian as $index => $data) : ?>
        <tr>
    ...
    Berdasarkan Data Diri Atas, Bahwa warga tersebut telah meninggal dunia pada :
    </p>
    <table style="margin-left:20;">
      <?php foreach ($kematian as $index => $data) : ?>
        <tr>
          ...
          <td style="width: 65%;"><b>
          <?php
          $date = date('Y-m-d', strtotime($data->tanggal_wafat));
          echo getDayIndonesia($date).' / '.date('j F Y', strtotime($data->tanggal_wafat));
          ?></b></td>
        </tr>
        <tr>
          <td style="width: 30%;">Lokasi Makam</td>
          <td style="width: 5%;">:</td>
          <td style="width: 65%;"><b><?= strtoupper($data->alamat_makam) ?></b></td>
        </tr>
      <?php endforeach ?>
    </table>
    <p align="justify" style="line-height: 30px;">
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Demikian Surat Keterangan Kematian ini kami buat 
